...
window.addEventListener('deviceorientation', function(e){
	yaw=compassHeading(e.alpha, e.beta, e.gamma);
    pitch=(e.beta-90)*Math.PI/180;
    document.getElementById("data").innerHTML=  round(yaw,3)+","+round(pitch,3);
}, false);
function compassHeading(alpha, beta, gamma) {
    ...
    return compassHeading; // Compass Heading (in degrees)
}
...
var cnum=-1;
var _touch=false;
canvas.addEventListener('touchstart',(e)=>(_touch=true ));
canvas.addEventListener('touchend'  ,(e)=>(_touch=false));
function roop(){
    var rotate=0.0;
    dealkeys();
    if(_touch)cood=vec3dPlus(cood,vec3dInnerProduct(new vec3d(Math.cos(yaw)*Math.cos(pitch),Math.sin(pitch),Math.sin(yaw)*Math.cos(pitch)),5));
